...
public class AlertaConfiguration : IEntityTypeConfiguration<Alerta>
{
    public void Configure(EntityTypeBuilder<Alerta> builder)
    {
        ...
        builder.Property(a => a.NrNivelRisco).HasColumnName("NR_NIVEL_RISCO").IsRequired();
        builder.Property(a => a.StStatus).HasColumnName("ST_STATUS").HasConversion<string>().HasMaxLength(20).IsRequired();
        builder.Property(a => a.DsObservacao).HasColumnName("DS_OBSERVACAO").HasMaxLength(1000);
        builder.Property(a => a.DtCriacao).HasColumnName("DT_CRIACAO").HasDefaultValueSql("SYSDATE").ValueGeneratedOnAdd();
        builder.Property(a => a.DtFechamento).HasColumnName("DT_FECHAMENTO");
